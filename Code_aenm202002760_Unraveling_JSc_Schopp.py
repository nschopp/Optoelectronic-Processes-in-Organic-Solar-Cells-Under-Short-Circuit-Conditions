...
print("Please refer to the publication for details on this work and for referencing:")
print(
    "\033[4m"
    + "A Simple Approach for Unraveling Optoelectronic Processes in Organic Solar Cells under Short‐Circuit Conditions"
    + "\033[0m"
...
thicknesses = [700000, 130, 42, 140, 7, 100]

plotGeneration = True  # toggle generation of graphs
plotWavelengths = [400, 500, 600]  # wavelengths to include in graphs
...
x_step = 1.0  # grid spacing of device cross section simulation (nm)
...
q = 1.60217657e-19  # electric charge (C)

# generate array of wavelengths to loop over
lambdas = np.arange(lambda_start, lambda_stop + lambda_step, lambda_step, float)

# generate array of layer thicknesses to loop over
...
# glass does not change much when included but slows down the calculation
# include for final calculation but not for testing
t[0] = 0  
Jsc = 0 * varyThickness
...
# Plot Current Density from 100% IQE
fig0 = plt.figure(0)
fig0.clf()
...
plt.savefig(savepath + "Current Density obtained from 100% IQE.tif")
fig0.show()
...
if plotGeneration:
    ...
    for matind in np.arange(1, len(t)):
        ...
        AbsRate = np.tile((a[matind, :] * np.real(n[matind, :])), (len(Pos), 1)) * (
            abs(E[Pos, :]) ** 2
        )
    ...
    plt.savefig(
        savepath + "Fraction of Light Absorbed or Reflected.tif",
        dpi=600,
        bbox_inches="tight",
    )
    ...
    plt.savefig(savepath + "Generation rate in device.tif")
    fig3.show()

    # Generation rate in active layer
    varyThickness_regrid = np.linspace(
        np.min(varyThickness), np.max(varyThickness), len(varyThickness) - 1
    )
    # plot
    fig4 = plt.figure(4)
    fig4.clf()
    ...
    plt.savefig(savepath + "Generation rate in active layer.tif")
    fig4.show()
...
# necessary for 3D plot, do not delete even if it shows that it is unused
...
plot = ax.plot_surface(
    X, Y, Z, cmap=plt.cm.jet, linewidth=0, edgecolor=None, rstride=1, cstride=1
)
fig.colorbar(plot, shrink=0.4, aspect=15, pad=0.05)
...
V_bi = 0.623  # can be obtained from JV curve fitting, see SI (V)
...
# set range in which to test value, start with wide range then narrow range down for results of higher accuracy
mutau_exponent = np.linspace(-9.4, -9, samples)
mutau = 10 ** (mutau_exponent)  # the mutau values that will be tested

L_cm = (thicknesses[activeLayer]) * 1e-7  # thickness of device (cm)
...
# import experimental EQE
EQE_array_raw = pd.read_csv((path_EQE), delimiter=",", header=0).values
all_wavelength_EQE = np.around(EQE_array_raw[:, 0])
all_EQE = EQE_array_raw[:stop_fit_index, 1] / 100

# bring EQE measured into the same shape as calculated EQE (determined by optical modeling wavelength)
EQE_measured_rough = np.zeros([len(wavelength), 1])
...
# smoothing measured EQE data, not always necessary
EQE_measured = savgol_filter(np.ravel(EQE_measured_rough), 21, 7)

# import AM1.5G spectrum
df_AM15 = pd.read_csv((path_AM15), delimiter=",", header=None)
...
# Filter: get the intensities from AM1.5G file for only those wavelength used in Optical Modeling
wl_and_I_needed = np.zeros([len(wavelength), 2])
for j in range(0, len(wavelength)):
...
I = wl_and_I_needed[:, 1]  # intensities of AM1.5G for the wavelength used in OM
...
for l in np.arange(0, len(wavelength)):
    for mt in np.arange(0, len(mutau)):
        for p in np.arange(0, len(x)):
            ...
            Jsc_ideal[l] = (
                np.sum(Gxl.T[l, :]) * 1e-7 * sc.e
            )  # 100% extaction and no geminate recmbination
            EQE_ideal[l] = (Jsc_ideal[l] / sc.e) / photonflux[l]

            Jsc_ideal_reduced_by_gem_rec[l] = (
                gRec * np.sum(Gxl.T[l, :]) * 1e-7 * sc.e
            )  # includes geminate recombinatin prefactor g, determinded separetly
            EQE_ideal_reduced_by_gem_rec[l] = (
                Jsc_ideal_reduced_by_gem_rec[l] / sc.e
            ) / photonflux[l]
            ...
            Jsc_hecht = (
                gRec * np.sum(Every_f[l, :]) * 1e-7 * sc.e
            )  # now integrated over x and only wavelength, mu, tau dependant
...
EQE_hecht_best_1 = Every_EQE_hecht[
    index_best_fit_1, :
]  # the EQE that gives the best fit
...
plt.plot(
    wavelength,
    EQE_ideal_reduced_by_gem_rec,
    "yellowgreen",
    linewidth=2,
    linestyle="-.",
    label=(r"$\eta$=1, $P_g$=" + str(gRec)),
)
plt.xlim(290, 1100)
...
plt.ylim(
    0, 0.9
)  # plt.plot(wavelength,  EQE_hecht_best_2, color= 'darkorange',  label='Hecht, Approach 2')
plt.plot(
    wavelength, EQE_hecht_best_1, color="red", linestyle="-.", linewidth=2, label="Fit"
)  # approahx 1b #from A to mA

plt.legend(loc=9, bbox_to_anchor=(0.5, -0.15), ncol=3)

plt.savefig(
    savepath + str(thicknesses) + "EQE_calc.tif",
    dpi=400,
    bbox_inches="tight",
)

output_EQEs = pd.DataFrame(
    {
        "wavelength in nm ": wavelength,
        "EQE measured": EQE_measured,
        "EQE eta=1, Pg=1": EQE_ideal,
        ("EQE eta=1, Pg=" + str(gRec)): EQE_ideal_reduced_by_gem_rec,
        "EQE Simulation Fit": EQE_hecht_best_1,
    }
)
...
plt.title(
    "Extraction efficiencies at 0 V, inerted structure",
    {"color": "black", "fontsize": 10},
)
plt.plot(
    x, Every_eta_n_0V, label="electron"
)  # choose any index of the samples just as exmple visualiztion
...
for p in np.arange(0, len(x)):
    w_cm_m3V = mutau_best_1 * (3 + V_bi) / (L_cm)  # Vbi plus applied voltage
...
plt.title(
    "Extraction efficiencies that lead to best fit, Appr. 1, at -3 V",
    {"color": "black", "fontsize": 10},
)
plt.plot(
    x, Every_eta_n_m3V, label="electron"
)  # choose any index of the samples just as exmple visualiztion
...
plt.title(
    "Extraction efficiencies at different thickness", {"color": "black", "fontsize": 10}
)
...
output = [
    "Simulation Fit N.S.:",
    "mobility-lifeftime-product (cmˆ2/Vs): ",
    "{:.2e}".format(mutau_best_1),
    "average_extraction_eff_at_minus_3_V approach 1 =",
    str("{:.3f}".format(average_extraction_eff_at_minus_3_V)),
    "r2=",
    str("{:.2f}".format(r_squared)),
]
